...
for (const [h, m, s] of [[17, 49, 0], [17, 57, 0], [17, 59, 29], [17, 59, 59]]) {
  ...
  check(`${t} percent < 100  ${r.percent}`, r.percent, (p) => p !== "100.00%");
}
...
check(`18:00:00 percent 100.00% ${done.percent}`, done.percent, "100.00%");
...
check(`…+4s of ticks -> 100.00% (${crossed.bar})`, crossed.percent, "100.00%");

console.log("\n=== Hundredths readout floors rather than rounds ===");
const almostDone = await snap(at(17, 59, 59), "09:00", "18:00");
check("17:59:59 still shows 99.xx%, never 100.00%", almostDone.percent,
  (p) => /^99\.\d{2}%$/.test(p) && p !== "100.00%");
const quarter = await snap(at(11, 15, 0), "09:00", "18:00");
check("11:15 -> exact 25.00%", quarter.percent, "25.00%");
...
check(`13:30 -> 50.00% (${mid.bar})`, mid.percent, "50.00%");
